...
<div class="row">
	...
	<div class="col-md-12">
		...
		<div class="box box-primary">	
			...
			<?php 
			$key = array_search('SetAttribute', $parametros);
			$setAttributeId = get_next($parametros, $key);
			if(isset($availableProductModel->id)){
			   
			   $formPost =  HOME_URI."/Products/RegisterProduct/Product/{$availableProductModel->id}/SetAttribute/{$setAttributeId}";
			}else{
			    $formPost =  HOME_URI."/Products/RegisterProduct/SetAttribute/{$setAttributeId}";
			}
			
			?>
			<form method="POST" action="<?php echo $formPost; ?>" name="form-register-product" enctype="multipart/form-data" >
			...
			<div class="box-body">
				<div class="row">
					<div class="col-md-6">
								...
								<div class="form-group">
		                            ...
		                            foreach($listBrands as $key => $brand){
										...
										if(isset($availableProductModel->brand) && !empty($availableProductModel->brand)){
										    $selected = strtoupper($brand['brand']) == strtoupper($availableProductModel->brand) ? "selected" : "" ;
										}else{
								...
								<div class="form-group">
		                            ...
		                            foreach($listColors as $key => $color){
										...
										if(isset($availableProductModel->color) && !empty($availableProductModel->color)){
										    $selected = strtoupper($color['color']) == strtoupper($availableProductModel->color) ? "selected" : "" ;
										}else{
						...
						<div class="form-group">
							<textarea name='description'  placeholder="Place some text here" style="width: 100%; height: 200px; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;"><?php echo  $availableProductModel->description; ?></textarea>
						</div>
						<div class="row">
							<div class="col-md-3">
								<div class="form-group">
					...
					<div class="row">
					    <div class="col-md-12">
    						<div class="form-group">
                    		<input type="text" class="form-control input-sm autocomplete_product_id" id='autocomplete_product_id'  product_id= '<?php echo $availableProductModel->id;?>' tabindex="5"  name='autocomplete-sku'  value=''>
							</div>
							...
							foreach($listRelational as $key => $value){
							    echo "<tr id='{$value['product_relational_id']}'>
                                        ...
                                        <td width='50px'>  
                                            <div class='form-group'>
                                                <input type='text' name='relational[{$value['product_id']}]' id='{$value['product_id']}' class='qtd_product_relational form-control input-sm'  value='{$value['qtd']}'>
                                            </div>
                                        </td>
			...
			<div class="box-footer">
				<button type="submit" class="btn btn-primary pull-right submit-load" name="generate-description">Gerar Descrição</button>
				<?php 
					if(isset($resPD['id'])){
