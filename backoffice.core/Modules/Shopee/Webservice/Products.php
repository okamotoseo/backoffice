                ...
                if(isset($rowParentId['parent_id'])){
                    ...
                    if(isset($verify['id_product'])){
                        ...
                        if(!empty($verify['id_product'])){
                            ...
                            $queryVariation = $db->query($sqlvariation);
                            $queryVar = $queryVariation->fetchAll(PDO::FETCH_ASSOC);
                            //                             pre($queryVar);die;
                            ...
                            foreach($availableVariations as $j => $variation){
                                ...
                                if(isset($variation['id']) and isset($images[0])){
                                    ...
                                    $sqlAP = "SELECT * FROM available_products WHERE store_id = {$storeId} AND id = {$variation['product_id']}";
                                    ...
                                    $salePriceModel->product_id = $verifyAP['product_id'];
                                    $salePrice = $salePriceModel->getSalePrice();
                                    $stockPriceRel = $salePriceModel->getStockPriceRelacional();
                                    $salePrice = empty($stockPriceRel['price']) ? $salePrice : $stockPriceRel['price'];
                                    $qtd = empty($stockPriceRel['qty']) ? $qtd : $stockPriceRel['qty'] ;
            ...
            while($row = $query->fetch(PDO::FETCH_ASSOC)){
                ...
                while($rowParent = $queryParent->fetch(PDO::FETCH_ASSOC)){
                    ...
                    if($storeId == 4){
                        ...
                        $price = ceil($salePrice * 1.3) - 0.10;
                        if($salePrice < $price){
                            
                            if ($salePrice > 17 && $salePrice < 79){
                            ...
                            $data["Product"]["promotional_price"] = ceil($salePrice) - 0.10;
                            $data["Product"]["start_promotion"] =  date("Y-m-d H:s:i", strtotime("NOW"));
                            $data["Product"]["end_promotion"] = date('2022-m-d H:s:i');
                            
                            echo $difPercent = (($price - $salePrice) * 100) / $price;
                            echo '<br>';
                            if($difPercent > 15 && $salePrice > 299){
                                $data["Product"]['hot'] = 1;
	                        ...
	                        if(!empty($rowVariations['sku'])){ 
	                            ...
	                            if($storeId == 4){
	                                ...
	                                if($salePrice < $price){
	                                    ...
	                                    $dataVariant["Variant"]["promotional_price"] = ceil($salePrice) - 0.10;
	                            ...
	                            if($qtd < 1){
	                                $dataVariant["Variant"]['hot'] = 0;
	                                $dataVariant["Variant"]["end_promotion"] =  date("Y-m-d H:s:i", strtotime("NOW"));
	                            }
