...
//                                 pre($product['sku']."  -  ".$productVariation['sku']);
        ...
        case "export_products":
            ...
            $typeRequest = isset($_REQUEST['type']) AND !empty($_REQUEST['type']) ? $_REQUEST['type'] : "";
            switch($typeRequest){
...
//             pre($productIds);die;
            foreach($productIds as $ind => $id){
                    ...
                    $sql = "SELECT id, sku, product_id FROM module_skyhub_products WHERE store_id = {$storeId} AND id = '{$id}'";
                    $query = $db->query($sql);
                    $products = $query->fetchAll(PDO::FETCH_ASSOC);
                    foreach($products as $key => $product){
