...
class AttributesRelationshipModel extends MainModel
{
    ...
    public function Save(){
        ...
        $sqlVerify = "SELECT * FROM `mg2_attributes_relationship` 
        ...
        if(!isset($resVerify['attribute_id'])){
            ...
            $query = $this->db->insert('mg2_attributes_relationship', array(
            ...
            $query = $this->db->update('mg2_attributes_relationship',
                array("store_id", "id"),
    ...
    public function TotalAttributes()
    {
        ...
        $sql = "SELECT count(*) as total FROM mg2_attributes_relationship WHERE store_id = ?";
    ...
    public function ListAttributesMg2()
    {
//         $sql = "SELECT * FROM mg2_attributes_relationship 
//         WHERE `store_id` = ? ORDER BY relationship DESC 
//         LIMIT {$this->linha_inicial},".QTDE_REGISTROS.";";
        
        $sql = "SELECT * FROM mg2_attributes_relationship
        WHERE `store_id` = ? ORDER BY is_configurable DESC;";
    ...
    public function ListSetAttributeRelationshipMg2()
    {
    	if(!isset($this->attribute_set_id)){
    		return array();
    	}
    	$sql = "SELECT * FROM mg2_set_attribute_relationships
        WHERE `store_id` = ? AND attribute_set_id = ? ORDER BY relationship DESC;";
    
    	$query = $this->db->query($sql, array($this->store_id, $this->attribute_set_id));
...
//         WHERE `store_id` = ?";
    ...
    public function GetAttributesRelationship()
    {
        ...
        $sql = "SELECT attribute_code, relationship FROM mg2_attributes_relationship
    ...
    public function Load()
    {
        if ( in_array('edit', $this->parametros )) {
            ...
            $query = $this->db->query('SELECT * FROM mg2_attributes_relationship WHERE store_id = ? AND `product_id`= ?', array($this->store_id, $id ) );
    ...
    public function Delete()
    {
        if ( in_array('del', $this->parametros )) {
            ...
            $query = $this->db->query('DELETE FROM mg2_attributes_relationship WHERE store_id = ? AND  `product_id`= ?', array($this->store_id, $id ) );
