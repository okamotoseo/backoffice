...
class SetupModel extends MainModel
{
	...
	public function Save(){
	    ...
	    $db_check_setup = $this->db->query (
	        'SELECT * FROM `module_sysemp` WHERE store_id = ?',
	        array($this->store_id)
	        );
	    ...
	    $storeId = $fetch_setup['store_id'];
	       ...
	       if ( ! empty( $storeId ) ) {
	           $query = $this->db->update('module_sysemp', 'store_id', $storeId, array(
	            'host' => $this->host,
	            'port' => $this->port,
	            'dbname' => $this->dbname,
	            'user' => $this->user,
	        ...
	        $query = $this->db->insert('module_sysemp', array(
	...
	public function Load()
	{
        ...
        if(!empty($this->store_id)){
            ...
            $query = $this->db->query('SELECT * FROM module_sysemp WHERE `store_id`= ?', array( $this->store_id ) );
