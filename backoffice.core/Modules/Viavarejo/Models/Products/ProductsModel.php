...
class ProductsModel extends MainModel
{
	...
	public function Save(){
	    ...
	    $query = $this->db->query("SELECT id FROM module_viavarejo_products
            WHERE store_id = {$this->store_id} AND product_id = '{$this->product_id}'");
	    $res = $query->fetch(PDO::FETCH_ASSOC);
	    
	    if ( ! empty( $res['id'] ) ) {
	        $query = $this->db->update('module_viavarejo_products',
	            array('store_id','id'),
	        ...
	        if($query->rowCount()){
	            ...
	            $this->db->update('module_viavarejo_products',
	                array('store_id','id'),
	                array($this->store_id, $res['id']),
	        ...
	        $query = $this->db->insert('module_viavarejo_products', array(
	...
	public function TotalProducts(){
	    ...
	    $sql = "SELECT count(*) as total FROM module_viavarejo_products WHERE store_id = {$this->store_id}";
	...
	public function ListProducts(){
        ...
        $query = $this->db->query("
    		SELECT available_products.*, module_viavarejo_products.id,module_viavarejo_products.sku, 
            module_viavarejo_products.parent_id, module_viavarejo_products.product_id
    		FROM module_viavarejo_products
    		LEFT JOIN available_products ON available_products.id = module_viavarejo_products.product_id
    		WHERE module_viavarejo_products.store_id= ?
    		ORDER BY module_viavarejo_products.parent_id DESC
	...
	public function GetProductsFilter()
	{
	    ...
	    foreach($class_vars as $key => $value){
	        if(!empty($this->{$key})){
	            switch($key){
	                case 'store_id': $where_fields .= "module_viavarejo_products.{$key} = {$this->$key} AND ";break;
	                case 'id': $where_fields .= "module_viavarejo_products.{$key} = {$this->$key} AND ";break;
	                case 'product_id': $where_fields .= "module_viavarejo_products.{$key} = {$this->$key} AND ";break;
	                case 'sku': $where_fields .= "module_viavarejo_products.{$key} LIKE '{$this->$key}' AND ";break;
	                case 'parent_id': $where_fields .= "module_viavarejo_products.{$key} LIKE '{$this->$key}' AND ";break;
	            }
	        }
	...
	public function GetProducts()
	{
	  ...
	  $sql = "SELECT available_products.*, module_viavarejo_products.id,module_viavarejo_products.sku, 
            module_viavarejo_products.parent_id, module_viavarejo_products.product_id
    		FROM module_viavarejo_products
    		LEFT JOIN available_products ON available_products.id = module_viavarejo_products.product_id
    		WHERE {$where_fields} ORDER BY module_viavarejo_products.parent_id DESC
            LIMIT {$this->linha_inicial}, {$this->records}";
	...
	public function Delete()
	{
        ...
        $query = $this->db->query('DELETE FROM module_viavarejo_products
        ...
        if ( ! $query ) {
            ...
            $query = $this->db->query('DELETE FROM module_viavarejo_products_variations
