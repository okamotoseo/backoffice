...
if(isset($storeId)){
    ...
    switch ($action){
    	...
    	case "update_stock_price";
	    	...
	    	$syncId =  logSyncStart($db, $storeId, "Viavarejo", $action, "Atualização do estoque e preço Viavarejo", $request);
    		...
    		case "get_seller_itens_status":
    			...
    			case "delete_products":
    				...
    				try {
    					...
    					$result = $loads->deleteProduct($sku);
    					
    					$sql = "DELETE FROM module_viavarejo_products WHERE store_id = {$storeId} AND sku LIKE '{$sku}'";
    					$db->query($sql);
    					...
    					if ($errors != null) {
    						foreach ($errors->errors as $error) {
    							echo ('http_status => '.$error->http_status .' '. $error->code . ' ' . $error->message . "\n");
    						}
    					} else {
    						$res = $e->getMessage();
    						pre($res);
