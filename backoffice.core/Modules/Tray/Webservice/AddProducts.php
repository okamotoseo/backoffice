    ...
    switch($action){
 		...
 		case "create_product_tray":
            ...
            foreach($productIds as $i => $id){
                ...
                $sql = "SELECT sku, title, reference, color, parent_id, reference, brand, collection 
                ...
                if($kit){
	                ...
	                if(isset($rowParentId['parent_id']) ){
	                    ...
	                    if(!empty($firstVar)){
	                        array_unshift($products, $firstVar);
	                    }
	                    foreach($products as $i => $value){
	                        if($value['quantity'] > 0){
	                            $availableVariations++;
