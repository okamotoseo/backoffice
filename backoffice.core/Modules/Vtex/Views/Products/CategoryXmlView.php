		...
		<div class='box box-primary'>
			...
			<div class="box-body no-padding">
	            ...
	            <?php 
	             ...
	             foreach ($listCategoriesXml as $fetch){
...
// 						}
						...
						echo "<tr {$styleParent} id='{$count}'>
							<td width='40%'>{$iconAngle} {$fetch['hierarchy']}</td>

							<td width='60%'>

								<select class='category select2 form-control' category='{$fetch['hierarchy']}' rowLine='{$count}' style='min-width:200px;'>
									 <option value='select'>Selecione</option>";
						
                    	             foreach($listDefaultCategoriesVtex as $key => $category){
                    	                 $selected = '' ;
                    	                 foreach($listCategoryRelationship as $i => $rel){
                    	                     if($fetch['hierarchy'] == $rel['hierarchy'] && $rel['id_category'] == $category['id']){
                    	                         $selected = 'selected' ;
                    	                         break;
                    	                     }
                    	                 }
