...
class RulesModel 
{
	    ...
	    if ( 'POST' == $_SERVER['REQUEST_METHOD'] && ! empty ( $_POST ) ) {
	        foreach ( $_POST as $property => $value ) {
	            if(!empty($value)){
	                ...
	                $arr = array('condition', 'percentage_rate', 'value_test', 'rule' );
	...
	public function Save(){
	    ...
	    if ( ! empty( $this->id ) ) {
	        ...
	        $query = $this->db->update('ml_price_rules', 'id', $this->id, array(
	            'condition' => $this->condition,
	            'operator' => $this->operator,
	            'value_test' => $this->value_test,
	            'rule' => $this->rule,
	        ...
	        if ( ! $query ) {
	            ...
	            $query = $this->db->insert('ml_price_rules', array(
	...
	public function ListRulesPrice()
	{
	    $query = $this->db->query('SELECT * FROM `ml_price_rules`  WHERE `store_id` = ? ORDER BY id DESC',
	        array($this->store_id)
	        );
	...
	public function Load()
	{
	    if ( chk_array( $this->parametros, 4 ) == 'edit' ) {
	        ...
	        $query = $this->db->query('SELECT * FROM ml_price_rules WHERE  store_id = ? AND `id`= ?', array($this->store_id, $id ) );
	...
	public function Delete()
	{
	    if ( chk_array( $this->parametros, 4 ) == 'del' ) {
	        ...
	        $query = $this->db->query('DELETE FROM ml_price_rules WHERE store_id = ? AND `id`= ?', array($this->store_id, $id ) );
