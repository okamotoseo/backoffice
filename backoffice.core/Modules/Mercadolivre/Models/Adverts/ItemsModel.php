...
//                 pre($item);die;
        ...
        if(file_exists($pathRead)){
            ...
            foreach ( $iterator as $key => $entry ) {
                ...
                if($file != '.' AND $file != '..'){
                    ...
                    $picturesArray[$array[0]] = array('source' =>  $pathShow.'/'.$file.'?'.time());
    ...
    public function getPathPictures(){
        $pictures = array();
        $pathShow = "https://backoffice.sysplace.com.br/Views/_uploads/store_id_{$this->store_id}/products/{$this->product_id}";
        $pathRead = "/var/www/html/app_mvc/Views/_uploads/store_id_{$this->store_id}/products/{$this->product_id}";
        if(file_exists($pathRead)){
            ...
            foreach ( $iterator as $key => $entry ) {
                ...
                if($file != '.' AND $file != '..'){
                    ...
                    $picturesArray[$array[0]] = array('source' =>  $pathRead.'/'.$file);
...
//                             $value  = !empty($attrValue['value']) ? $attrValue['value'] : $variation['color'] ;
                            $value  = !empty($variation['color']) ? $variation['color'] : $attrValue['value'] ;
...
//                             $pathShow = "https://backoffice.sysplace.com.br/Views/_uploads/store_id_{$this->store_id}/products/{$variation['id']}";
                            ...
                            $pathShow = "/var/www/html/app_mvc/Views/_uploads/store_id_{$this->store_id}/products/{$variation['id']}";
                            ...
                            if(file_exists($pathRead)){
                                ...
                                if(!isset($color)){
                                    ...
                                    $this->variations["{$ind}"]["picture_ids"] =  array($color => $colorsImages);
...
//                             if(!isset($tags->defines_picture)){
                                ...
                                $valueName = '';
                                $attrId = trim($attrValue['attribute_id']);
                                switch($attrId){
//                                     case "STRUCTURE_COLOR": 
//                                         $valueName = $variation['color']; 
//                                         $valueName = ucwords(implode(" ", array_unique(explode("/", $valueName))));
//                                         $valueName = ucwords(implode(" ", array_unique(explode("-", $valueName))));
//                                         break;
                                    case "VOLTAGE": $valueName = str_replace(' ', '', $variation['variation']); break;
                                    case "FLAVOR": $valueName = str_replace(' ', '', $variation['color']); break;
                                    case "OPERATING_VOLTAGE": $valueName = str_replace(' ', '', $variation['variation']); break;
