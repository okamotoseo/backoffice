...
class ImportAdvertsModel extends MainModel
{
...
// 	        echo 123;die;
	...
	public function TotalAdverts(){
       ...
       $sql = "SELECT count(DISTINCT ml_products_attributes.product_id) as total FROM  ml_products_attributes WHERE {$where_fields}";
	...
	public function ListMlProducts(){
	    ...
	    foreach($res as $key => $row){
            ...
            AND ml_products_attributes.product_id = {$row['product_id']}
...
// 	           pre($variation);die;
	...
	public function GetMlProductsFilter()
	{
	    ...
	    foreach($class_vars as $key => $value){
	        if(!empty($this->{$key})){
	            switch($key){
	                case 'store_id': $where_fields .= "ml_products_attributes.{$key} = {$this->$key} AND ";break;
	                case 'product_id': $where_fields .= "ml_products_attributes.{$key} = '{$this->$key}' AND ";break;
	                case 'sku': $where_fields .= "ml_products_attributes.{$key} LIKE '{$this->$key}' AND ";break;
