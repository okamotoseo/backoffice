...
class SuscriptionsModel 
{
	...
	public function Save(){
	    ...
	    $db_check_suscriptions = $this->db->query (
	        'SELECT * FROM `module_mercadopago` WHERE store_id = ?',
	        array($this->store_id)
	        );
	    
	    if ( ! $db_check_suscriptions ) {
	        $this->form_msg = '<div class="alert alert-danger alert-dismissable">Internal error.</div>';
	        return;
	    }
	    
	    $fetch_suscriptions = $db_check_suscriptions->fetch();
	    $storeId = $fetch_suscriptions['store_id'];
