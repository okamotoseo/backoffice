...
class OrderPaymentsModel extends MainModel
{
    ...
    public function Save(){
        ...
        $query = $this->db->query('SELECT id FROM `order_payments`  WHERE  `store_id` = ?
    				AND PedidoId = ? AND PagamentoId = ? ORDER BY id DESC',
            array($this->store_id, $this->PedidoId, $this->PagamentoId)
            );
        
        $res = $query->fetch(PDO::FETCH_ASSOC);
        if(!empty($res['id'])){
            $query = $this->db->update('order_payments', 'id', $res['id'], array(
